...
filename = '081819_cell3_0mV.18Aug19.S1.E3';


%filename = '070919_cell11_0.09Jul19.S1.E3';
...
legend('raw', 'filtered')
  ...
  %axis([23000 25000 -100 100])
% 
  %axis([55000 60000 -100 100])
...
%     [max_val, col] = max(filt_trace(events(event_ind(ii))+1:events(event_ind(ii+1))));
%     plot(time(events(event_ind(ii))+col), max_val, 'r.')
...
%         ind_min = find(ss>0.1*max_val);
%         ind_max = find(ss>0.9*max_val);
%         rise_time = (ind_max(end)-ind_min(1))/10;
%         [fitres, gof] = exp_tau(filt_trace(events(event_ind(ii))+col+2:min(events(event_ind(ii))+col+12, length(filt_trace))), plotflag);
%         event_info = [event_info; time(events(event_ind(ii))+col), max_val, rise_time, -fitres.a];
%     end
% end
% 
% save(['/nas/data1/Ben/patchclamp/pcdh/20Aug19/', filename, 'event_info.mat'], 'event_info')
